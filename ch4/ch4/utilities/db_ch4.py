...
def fetch_metadata_for_single_channel(channel_id: int) -> list[dict]:
    with engine.connect() as conn:
        rp = conn.execute(
            sa.select(channel_metadata_table)
...
channel_message_table_name = config["telegram-db"]["channel-messages-table"]
...
messages_queue = config["rabbit-mq"]["telegram-messages-queue"]
